...
# BOT that checks every 1 seconde if the Liquidity Pool MATIC(ETH)/Token has been created and buy if it's the case
def checkReserve():
    ...
    while True:
        try:
            ...
            #if amounts_out>0, the LP has been created. The BOT buy
            if amounts_out>0:
                ...
                print('\n', 'buy')
                print('value ', str(valueETH_to_buy))
                getAmountsOut = getAmountOutMatictokens(valueETH_to_buy)[1]
                minout = float(getAmountsOut) * 0.999
                minout = web3.toWei(minout, 'wei')
                print('minout: ',minout)
                timestamp = gettimestamp() + (60 * 5)

                transaction = contract.functions.swapExactETHForTokens(minout,[wmatic_add,token_address],wallet, timestamp).buildTransaction({
                    'gas': 1000000,
                    'gasPrice': web3.toWei(gas_, 'gwei'),
                    'from': wallet,
                    'value': web3.toWei(valueETH_to_buy, 'ether'),
                    'nonce': nonce,
                })
                #print(transaction)
