...
def getbalanceMatic(_wallet):
    ...
    return callBalance/1000000000000000000
#getbalanceMatic()
...
def add_LP():
    ...
    valueETH=100
    balanceMatic = getbalanceMatic(wallet)
    balanceToken = getbalanceToken(wallet)

    # add LP
    print('\n', 'Add Liquidity')

    gas = web3.eth.gas_price
    #Increase by 20% to be executed immediately
    gas_ = round(gas * 1.2 / 1000000000)
    print("gas_", gas_)

    nonce = web3.eth.getTransactionCount(wallet)

    getAmountsOut = getAmountOutMatictokens(valueETH)[1]
    minout = float(getAmountsOut) * 0.98
    minout = web3.toWei(minout, 'wei')
    desired = float(getAmountsOut) * 1.02
    desired = web3.toWei(desired, 'wei')
    print('Value ETH / Tokens: ',valueETH, " / ", minout)

    if balanceMatic>=valueETH and balanceToken>=desired * 10**18:
        timestamp = gettimestamp() + (60 * 5)

        transaction = contract.functions.addLiquidityETH('token_address', desired, minout, valueETH, wallet, timestamp).buildTransaction({
                   'gas': 500000,
                   'gasPrice': web3.toWei(gas_, 'gwei'),
                   'from': wallet,
                   'value': web3.toWei(valueETH, 'ether'),
                   'nonce': nonce,
        })
        #print(transaction)
        ...
        print("Transaction hash ",json_obj[len(json_obj)-4])
